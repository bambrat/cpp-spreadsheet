...
namespace ASTImpl {
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const std::function<double(Position)>& args) const override {
                ...
                case Multiply:
                    ...
                    break;
                case Divide:
                    auto check_rhs_ = rhs_->Evaluate(args);
                    if (check_rhs_ == 0) {
                        throw FormulaError{ FormulaError::Category::Div0 };
                    }
                    result = lhs_->Evaluate(args) / check_rhs_;
                    break;
                }
                if (std::isinf(result)) {
                    throw FormulaError{ FormulaError::Category::Div0 };
        ...
        public:
            ...
            void Print(std::ostream& out) const override {
                if (!cell_->IsValid()) { out << FormulaError::Category::Ref; }
                else { out << cell_->ToString(); }
            }
...
FormulaAST ParseFormulaAST(std::istream& in) {
    ...
    auto ret = FormulaAST(listener.MoveRoot(), listener.MoveCells());
    return ret;
}
